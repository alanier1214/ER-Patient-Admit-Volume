...
Timetable.Sites <- crossing(Site = Sanford_Sites, DateTime = Timetable$DateTime)
...
``` {r Filtered_Data}
# ========== Timetables ==========
Timetable.Year <- Sanford_ED.Timetable %>%
  group_by(Site, Year) %>%
...
Timetable.Month <- Sanford_ED.Timetable %>%
  group_by(Site, Month) %>%
  summarise(Events = sum(Events),
            across(ends_with("Sum"), sum),
            across(ends_with("Avg"), mean),
...
Timetable.Day <- Sanford_ED.Timetable %>%
  group_by(Site, Day) %>%
  summarise(Events = sum(Events),
            across(ends_with("Sum"), sum),
            across(ends_with("Avg"), mean),
...
Timetable.Shift <- Sanford_ED.Timetable %>%
  group_by(Site, Shift) %>%
  summarise(Events = sum(Events),
            across(ends_with("Sum"), sum),
            across(ends_with("Avg"), mean),
...
Timetable.Hour <- Sanford_ED.Timetable %>%
  group_by(Site, Hour) %>%
  summarise(Events = sum(Events),
            across(ends_with("Sum"), sum),
            across(ends_with("Avg"), mean),
...
# Total Admissions per hour; Continous (Not very useful)
Trend.Admissions_sum.LC <- Sanford_ED.Timetable %>%
  ggplot(aes(
    x=DateTime,
    y=Admissions.Sum,
    group = Site,
